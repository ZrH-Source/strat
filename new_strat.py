...
message = f"Coin Balance : {symbol_balance}\nAll Balance (USDT) : {usdt_all_balance}"
    ...
    if symbol_balance[symbol] == 0:
        if row["super_trend_direction"] == True and row["ema_short"] > row["ema_long"]:
            ...
            try:
                ...
                message = f"Buy limit on {pair} of {buy_quantity} at the price of {buy_limit_price}$ \n Balance = {usdt_all_balance}"
                url = f"https://api.telegram.org/bot{TOKEN}/sendMessage?chat_id={chat_id}&text={message}"
                print(requests.get(url).json())
        ...
        if row["super_trend_direction"] == False or row["ema_short"] < row["ema_long"]:
            ...
            try:
                ...
                message = f"Sell limit on {pair} of {sell_quantity} at the price of {sell_limit_price}$ \n  Balance = {usdt_all_balance}"
                url = f"https://api.telegram.org/bot{TOKEN}/sendMessage?chat_id={chat_id}&text={message}"
                print(requests.get(url).json())
            except Exception as e:
                message = f"    Error: {e}"
